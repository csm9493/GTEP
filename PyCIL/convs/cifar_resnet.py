...
class CifarResNet_Large(nn.Module):
    """
    ResNet optimized for the Cifar Dataset, as specified in
    https://arxiv.org/abs/1512.03385.pdf
    """
    ...
    def __init__(self, block, depth, channels=3):
        super(CifarResNet_Large, self).__init__()

        # Model type specifies number of layers for CIFAR-10 and CIFAR-100 model
        assert (depth - 2) % 6 == 0, 'depth should be one of 20, 32, 44, 56, 110'
        layer_blocks = (depth - 2) // 6

        self.conv_1_3x3 = nn.Conv2d(channels, 50, kernel_size=3, stride=1, padding=1, bias=False)
        self.bn_1 = nn.BatchNorm2d(50)

        self.inplanes = 50
        self.stage_1 = self._make_layer(block, 50, layer_blocks, 1)
        self.stage_2 = self._make_layer(block, 100, layer_blocks, 2)
        self.stage_3 = self._make_layer(block, 200, layer_blocks, 2)
        self.avgpool = nn.AvgPool2d(8)
        self.out_dim = 200 * block.expansion
        self.fc = nn.Linear(200*block.expansion, 10)
...
def resnet32_large():
    """Constructs a ResNet-32 model for CIFAR-10."""
    model = CifarResNet_Large(ResNetBasicblock, 32)
    return model
