...
class PODNet(BaseLearner):
    def __init__(self, args):
        ...
        self.lambda_c_base = args["lambda_c"]
        self.lambda_f_base = args["lambda_f"]
        self.nb_proxy = args["nb_proxy"]
        self.ft_epochs = args["ft_epochs"]
        self.ft_lrate = args["ft_lrate"]
        self.ft_milestones = args["ft_milestones"]
        self.using_adaptive_factor = args["adaptive_factor"]
        
        self._network = CosineIncrementalNet(
            args, pretrained=False, nb_proxy=self.nb_proxy
        )
    ...
    def _train(self, data_manager, train_loader, test_loader):
        ...
        if self._cur_task == 0:
            ...
            self._run(train_loader, test_loader, optimizer, scheduler, init_epoch)
        else:
            optimizer = optim.SGD(
            ...
            self._run(train_loader, test_loader, optimizer, scheduler, self.epochs)
        ...
        network_params = [
            {"params": base_params, "lr": self.ft_lrate, "weight_decay": self.weight_decay},
        ...
        if self.scheduler == 'cosine':
            ...
            scheduler = optim.lr_scheduler.MultiStepLR(
                optimizer=optimizer, milestones=self.ft_milestones, gamma=self.lrate_decay
            )

        self._run(finetune_train_loader, test_loader, optimizer, scheduler, self.ft_epochs)
        ...
        for epoch in range(1, epk + 1):
            ...
            for i, (_, inputs, targets) in enumerate(train_loader):
                ...
                if self._old_network is not None:
                    ...
                    spatial_loss = (
                        pod_spatial_loss(fmaps, old_fmaps) * self.factor * self.lambda_c_base
                    )
