...
class MEMO(BaseLearner):
    ...
    def __init__(self, args):
        ...
        self.examplar_batch_size = args["examplar_batch_size"]
    ...
    def _update_representation(self, train_loader, test_loader, optimizer, scheduler):
        ...
        for _, epoch in enumerate(prog_bar):
            ...
            if epoch%5==0:
                ...
                self._cur_task, epoch+1, self.epochs, losses/len(train_loader),losses_clf/len(train_loader),losses_aux/len(train_loader),train_acc, test_acc)
            else:
                info = 'Task {}, Epoch {}/{} => Loss {:.3f}, Loss_clf {:.3f}, Loss_aux {:.3f}, Train_accy {:.2f}'.format(
                self._cur_task, epoch+1, self.epochs, losses/len(train_loader), losses_clf/len(train_loader),losses_aux/len(train_loader),train_acc)
            prog_bar.set_description(info)
        logging.info(info)
    ...
    def _construct_exemplar(self, data_manager, m):
        ...
        for class_idx in range(self._known_classes, self._total_classes):
            ...
            idx_loader = DataLoader(
                idx_dataset, batch_size=self.examplar_batch_size, shuffle=False, num_workers=4
            )
